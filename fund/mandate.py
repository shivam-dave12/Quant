...
class FundMandate:
    ...
    top_n_execution_desks: int = 0
    top_n_depth_scan: int = 0
    ...
    max_spread_bps_option: float = 120.0
    max_spread_bps_future: float = 65.0
    ...
    def from_config(cls) -> "FundMandate":
        return cls(
            ...
            top_n_execution_desks=max(0, int(_cfg("FUND_TOP_N_EXECUTION_DESKS", 0))),
            top_n_depth_scan=max(0, int(_cfg("FUND_TOP_N_DEPTH_SCAN", 0))),
            ...
            max_spread_bps_option=float(_cfg("FUND_MAX_SPREAD_BPS_OPTION", 120.0)),
            max_spread_bps_future=float(_cfg("FUND_MAX_SPREAD_BPS_FUTURE", 65.0)),
    ...
    def max_spread_for_class(self, asset_class: str) -> float:
        ...
        if "option" in ac:
            return self.max_spread_bps_option
        if "future" in ac:
            return self.max_spread_bps_future
        if "commodity" in ac:
