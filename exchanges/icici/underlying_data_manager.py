...
class ICICIUnderlyingDataManager:
    ...
    def _maybe_refresh_live(self) -> None:
        ...
        if not live:
            ...
            try:
                self._sio.ensure_live(max_stale_sec=float(_cfg("ICICI_INDEX_STREAM_MAX_STALE_SEC", 15.0)), reason="underlying_transport_health_check")
            except Exception:
                pass
    ...
    def _repair_stream_if_stale(self, reason: str, *, wait: bool = False) -> bool:
        if not bool(_cfg("ICICI_INDEX_STREAM_ENABLED", True)) or not bool(_cfg("ICICI_INDEX_WEBSOCKET_REQUIRED", True)):
            return False
        ...
        if not session.is_open and bool(_cfg("ICICI_ANALYZE_ONLY_DURING_MARKET_SESSION", True)):
            return False
        now = time.time()
        max_stale = float(_cfg("ICICI_INDEX_STREAM_MAX_STALE_SEC", 15.0))
        with self._lock:
            stream_age = now - float(self._last_stream_tick_ts or 0.0) if self._last_stream_tick_ts else 999999.0
        ...
        if ok:
            logger.warning(
                "ICICI underlying websocket repair triggered for %s after %.1fs without a fresh tick",
