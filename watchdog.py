...
class EntryEngineStuckCheck(HealthCheck):
    """
    The entry engine has structural states SCANNING / CONTEXT_READY / LIQUIDITY_RAID / EXECUTABLE / ENTERING / IN_POSITION.
    ...
    We detect:
      * bot `_pos.phase == FLAT` but entry_engine._state is NOT in
        {SCANNING, CONTEXT_READY, LIQUIDITY_RAID, EXECUTABLE}. This means the entry engine is still in
    ...
    def check(self) -> HealthResult:
        ...
        if ee_state_name in ("SCANNING", "CONTEXT_READY", "LIQUIDITY_RAID", "EXECUTABLE"):
        ...
        return self._heal(
            ...
            f"— all new structural raids will be rejected until self-recovery",
            action="reset_entry_engine",
            age_sec=round(age, 1), engine_state=ee_state_name,
        )
