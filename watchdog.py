...
class CircuitBreaker:
    ...
    When engaged, the breaker:
      1. sets `strategy.watchdog_trading_frozen = True` (the strategy decision state
    ...
    def trip(self, reason: str, auto: bool = False) -> None:
        ...
        msg = (f"🛑 <b>WATCHDOG CIRCUIT BREAKER — {mode}</b>\n"
               ...
               f"New entries FROZEN. Existing exchange protection and reconciliation remain active.\n"
...
class StuckPosSyncFlagCheck(HealthCheck):
    ...
    so partial-target reconciliation and active exposure observation stop running.
    """
    ...
    def check(self) -> HealthResult:
        ...
        return self._heal(
            ...
            f"— active protection reconciliation is BLOCKED until this clears",
...
class MissingSlOrderIdCheck(HealthCheck):
    """
    Detect a broker-protected position whose tracked structural stop identifier
    is absent. Recovery queries exchange open orders and adopts the matching
    reduce-only stop on the correct side without changing its geometry.
    """
    ...
    def check(self) -> HealthResult:
        ...
        return self._heal(
            f"sl_order_id EMPTY for {age:.0f}s — structural protection tracking is incomplete "
...
class StrategyDecisionStallCheck(HealthCheck):
    """
    The strategy decision state has structural states SCANNING / CONTEXT_READY / LIQUIDITY_liquidity_event / EXECUTABLE / ENTERING / IN_POSITION.
    ...
    We detect:
      * bot `_pos.phase == FLAT` but entry_engine._state is NOT in
        {SCANNING, CONTEXT_READY, LIQUIDITY_liquidity_event, EXECUTABLE}. This means the strategy decision state is still in
    ...
    def check(self) -> HealthResult:
        ...
        if pos is None or engine is None:
            return self._ok("no strategy decision state")
        ...
        if signal is not None:
            ...
            if age < 60.0:
                return self._info(
                    f"strategy decision state has pending {entry_type_name} {side} signal "
            ...
            return self._heal(
                f"strategy decision state has stale {entry_type_name} {side} signal "
        ...
        if ee_state_name in ("SCANNING", "CONTEXT_READY", "LIQUIDITY_liquidity_event", "EXECUTABLE"):
        ...
        if age < 60.0:
            return self._info(
                f"strategy decision state={ee_state_name} but bot=FLAT ({age:.0f}s — within grace)",
                age_sec=round(age, 1), engine_state=ee_state_name,
            )

        return self._heal(
            f"strategy decision state stuck in {ee_state_name} for {age:.0f}s while bot=FLAT "
            f"— all new structural liquidity_events will be rejected until self-recovery",
            action="reset_entry_engine",
            age_sec=round(age, 1), engine_state=ee_state_name,
        )
...
class NoTradesAfterFirstCheck(HealthCheck):
    ...
    needed to diagnose WHY (strategy decision state state, recent rejections, risk
    gate state, etc.).
    """
...
class DailyCounterConsistencyCheck(HealthCheck):
    ...
    def check(self) -> HealthResult:
        ...
        # captured in institutional_bot.log for forensic review, but no Telegram
        ...
        # INFO-level observation — shows up in institutional_bot.log for
...
class ResetLastExitTimeAction(HealAction):
    ...
    def apply(self, ctx: HealContext) -> Tuple[bool, str]:
        try:
            ...
            cooldown = float(_safe_getattr(cfg, "MIN_TIME_BETWEEN_TRADES_SEC", 30.0) or 30.0)
    ...
    def apply(self, ctx: HealContext) -> Tuple[bool, str]:
        ...
        def _do_reset() -> None:
            ...
            try:
                ...
                from strategy.institutional_strategy import PositionPhase  # type: ignore
            pos = _safe_getattr(strat, "_pos", None)
            if pos is not None:
        ...
        try:
            ...
            return True, "phase forced FLAT, strategy decision state reset"
        except Exception as e:  # noqa: BLE001
            return False, f"exception: {e}"
            ...
            if callable(finalise):
                try:
                    ...
                    try:
                        ...
                        from strategy.institutional_strategy import PositionState  # type: ignore
...
class ResetStrategyDecisionStateAction(HealAction):
    """
    ...
    def apply(self, ctx: HealContext) -> Tuple[bool, str]:
        ee = _safe_getattr(ctx.strategy, "_entry_engine", None)
        if ee is None:
            return False, "no strategy decision state"
        try:
...
class AutoHealRegistry:
    ...
    def default(cls) -> "AutoHealRegistry":
        ...
        for action in (
            ...
            ResetStrategyDecisionStateAction(),
        ):
            reg.register(action)
        return reg
...
class Watchdog:
    ...
    # still log to institutional_bot.log via the WARN result for forensic value, but
    ...
    def _register_default_checks(self) -> None:
        ...
        self._checks.extend([
            ...
            # L3
            StrategyDecisionStallCheck(**common),
...
# ═════════════════════════════════════════════════════════════════════════════
#
# For the circuit breaker to actually block new entries, institutional_strategy.py
