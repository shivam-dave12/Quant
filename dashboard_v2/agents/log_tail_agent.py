...
DIR = re.compile(r"\[(?P<asset>[^\|]+)\|(?P<venue>[^:]+):(?P<symbol>[^\]]+)\].*DIR_TELEMETRY: hunt=(?P<hunt>\w+) conf=(?P<conf>[0-9.]+)")
SPREAD = re.compile(r"\[(?P<asset>[^\|]+)\|(?P<venue>[^:]+):(?P<symbol>[^\]]+)\].*Spread cost impairment.*ratio=(?P<ratio>[0-9.]+).*spread=(?P<bps>[0-9.]+)bps.*ATR=\$(?P<atr>[0-9.]+).*size_mult=(?P<size>[0-9.]+)")
...
TRAIL = re.compile(r"\[(?P<asset>[^\|]+)\|(?P<venue>[^:]+):(?P<symbol>[^\]]+)\].*(?:Trailing|TRAIL|SL replaced|SL edited).*?(?:SL|stop)=?\$?(?P<sl>[0-9,\.]+)?", re.I)
...
def parse(line: str) -> Optional[dict[str, Any]]:
    ...
    if m := CATALOG.search(s):
        ...
        return {"type": "catalog_asset", "asset": m.group('asset'), "venue": m.group('primary').upper(), "symbol": symbol, "primary": m.group('primary'), "last_reason": venues}
    if m := AGG.search(s):
        return {"type": "market_data", "asset": m.group('asset').upper(), "primary": m.group('primary'), "secondary": m.group('secondary'), "data_status": "INITIALISED"}
    if m := STRAT.search(s):
        ev = base_ctx(m); ev.update({"type": "market_data", "leverage": m.group('leverage'), "margin": m.group('margin'), "data_status": "STRATEGY_READY"}); return ev
    if m := SCAN.search(s):
        ev = base_ctx(m); ev.update({"type": "scan", "price": fnum(m.group('price')), "state": m.group('state'), "phase": m.group('state'), "open_positions": int(m.group('open')), "max_positions": int(m.group('max'))}); return ev
    if m := DIR.search(s):
        ev = base_ctx(m); ev.update({"type": "direction", "direction": m.group('hunt'), "confidence": fnum(m.group('conf')), "phase": "DIRECTION"}); return ev
    if m := SPREAD.search(s):
        ev = base_ctx(m); ev.update({"type": "spread", "spread_atr": fnum(m.group('ratio')), "spread_bps": fnum(m.group('bps')), "atr": fnum(m.group('atr')), "size_mult": fnum(m.group('size')), "phase": "COST_IMPAIRMENT"}); return ev
    if "CANDIDATE DEFERRED" in s:
        m = CTX.search(s)
        ev = base_ctx(m) if m else {"asset":"","venue":"","symbol":""}
        ev.update({"type": "candidate_deferred", "phase": "DEFERRED", "last_reason": s.split('CANDIDATE DEFERRED:',1)[-1].strip(), "message": s}); return ev
    if "ENTRY CANDIDATE APPROVED" in s:
        m = CTX.search(s)
        ev = base_ctx(m) if m else {"asset":"","venue":"","symbol":""}
        ev.update({"type": "candidate_approved", "phase": "APPROVED", "message": s}); return ev
    if m := ENTER.search(s):
    ...
    if m := TRAIL.search(s):
        ev = base_ctx(m); ev.update({"type": "trail_update", "trailing": "ON", "sl": fnum(m.group('sl')) if m.group('sl') else None}); return ev
    if m := EXIT.search(s):
    ...
    if any(x in s for x in ["ERRO", "ERROR", "Traceback", "NameError", "TypeError", "AttributeError"]):
        m = CTX.search(s)
        ev = base_ctx(m) if m else {"asset":"","venue":"","symbol":""}
        ev.update({"type": "error", "severity": "critical", "title": "Runtime error", "message": s[:1200]}); return ev
    if "WARN" in s or "WARNING" in s:
        m = CTX.search(s)
        ev = base_ctx(m) if m else {"asset":"","venue":"","symbol":""}
        ev.update({"type": "warning", "severity": "warning", "title": "Warning", "message": s[:1200]}); return ev
    return None
...
class Poster:
    ...
    def post(self, ev: dict[str, Any]) -> None:
        ...
        try: requests.post(self.url + '/api/events', json=ev, timeout=1.5)
        except Exception: pass


def run(path: Path, dashboard: str, from_start: bool) -> None:
    poster = Poster(dashboard)
    poster.post({"type":"heartbeat", "mode":"live", "source":"log-tail"})
