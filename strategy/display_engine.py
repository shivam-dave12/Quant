# -*- coding: utf-8 -*-
"""
strategy/display_engine.py — Institutional Display Engine (terminal + Telegram)
==========================================================================
...
def format_periodic_report_display(
    price: float,
    balance: float,
    atr: float,
    engine_state: str,
...
def format_entry_alert_display(
    side: str,
    entry_type: str,
    entry_price: float,
    sl_price: float,
    ...
    et = entry_type.upper()
    if et == "APPROACH":
        lines.append("  Pre-sweep approach is disabled; executable entries require a confirmed sweep + posterior acceptance.")
    elif et == "REVERSAL":
...
def format_status_report_display(
    price: float,
    atr: float,
    atr_pctile: float,
    balance: float,
