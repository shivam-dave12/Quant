...
_TP_REASONABLE_MIN_ATR   = 0.75
_TP_REASONABLE_IDEAL_ATR = 3.25
_TP_REASONABLE_SOFT_MAX_ATR = 10.0

# SL distance preference. The executable stop must still be outside structural
# invalidation/protective liquidity; this only prevents a far, high-significance
# pool from winning automatically when a closer valid liquidity shield exists.
_SL_REASONABLE_MIN_ATR   = 0.65
_SL_REASONABLE_IDEAL_ATR = 2.75
_SL_REASONABLE_SOFT_MAX_ATR = 8.0
...
def _tp_reasonable_distance_mult(distance_atr: float, tf: str = "",
                                  terminal_target: bool = False) -> float:
    """Soft preference for executable TP distance, not nearest-pool bias.

    A target should be far enough to pay fees/slippage and give the trade room
    to breathe, but not so far that it is only a theoretical runner. HTF/terminal
    pools keep a higher floor so real external liquidity is not discarded.
    """
    d = max(float(distance_atr), 0.0)
    if d <= 0.0:
        return 0.50
    if d < _TP_REASONABLE_MIN_ATR:
        # Near pools can still win, but they need exceptional probability/EV.
        return 0.62 + 0.28 * (d / max(_TP_REASONABLE_MIN_ATR, 1e-9))
    if d <= _TP_REASONABLE_IDEAL_ATR:
        return 0.90 + 0.25 * ((d - _TP_REASONABLE_MIN_ATR) / max(_TP_REASONABLE_IDEAL_ATR - _TP_REASONABLE_MIN_ATR, 1e-9))
    if d <= _TP_REASONABLE_SOFT_MAX_ATR:
        return 1.15 - 0.20 * ((d - _TP_REASONABLE_IDEAL_ATR) / max(_TP_REASONABLE_SOFT_MAX_ATR - _TP_REASONABLE_IDEAL_ATR, 1e-9))
    tf_floor = 0.58 if (_tf_rank(tf) >= _TERMINAL_TP_MIN_TF_RANK or terminal_target) else 0.38
    excess = d - _TP_REASONABLE_SOFT_MAX_ATR
    return max(tf_floor, 0.95 / (1.0 + 0.055 * excess * excess))


def _tp_selection_value(ev: float, rr: float, rr_floor: float,
                        distance_atr: float, tf: str = "",
                        terminal_target: bool = False) -> float:
    surplus_r = max(float(rr) - float(rr_floor), 0.0)
    payoff_frontier = math.sqrt(max(float(rr), 0.0)) * (1.0 + min(surplus_r * 0.25, 1.00))
    delivery_span = 1.0 + min(max(float(distance_atr) - 1.0, 0.0) * 0.030, 0.24)
    reasonable_distance = _tp_reasonable_distance_mult(distance_atr, tf, terminal_target)
    return float(ev) * payoff_frontier * delivery_span * reasonable_distance
...
def score_tp_pools(
    ...
    for target in pools:
        try:
            ...
            selection_ev = _tp_selection_value(ev, rr, rr_floor, dist_atr, tf, terminal_target)
            ...
            _dist_mult = _tp_reasonable_distance_mult(dist_atr, tf, terminal_target)
            if _dist_mult < 0.90:
                reasons.append(f"distance-adjusted×{_dist_mult:.2f}")
            ...
            out.append(PoolScore(
                ...
                components   = {
                    ...
                    "reasonable_distance": _tp_reasonable_distance_mult(dist_atr, tf, terminal_target),
...
def _sl_reasonable_distance_mult(distance_atr: float, tf: str) -> float:
    """Soft preference for SL anchors at a reasonable protective distance.

    This does not remove far HTF protection; it prevents the selector from
    blindly choosing the farthest/strongest pool when a closer valid liquidity
    shield gives the same thesis protection with less capital drag.
    """
    d = max(float(distance_atr), 0.0)
    if d <= 0.0:
        return 0.55
    if d < _SL_REASONABLE_MIN_ATR:
        return 0.70 + 0.20 * (d / max(_SL_REASONABLE_MIN_ATR, 1e-9))
    if d <= _SL_REASONABLE_IDEAL_ATR:
        return 0.90 + 0.20 * ((d - _SL_REASONABLE_MIN_ATR) / max(_SL_REASONABLE_IDEAL_ATR - _SL_REASONABLE_MIN_ATR, 1e-9))

    # Higher timeframe anchors are allowed to sit farther away before the
    # distance haircut becomes meaningful. This keeps macro invalidation usable
    # while still favouring reasonable executable risk.
    soft_max = _SL_REASONABLE_SOFT_MAX_ATR + 1.0 * max(0, _tf_rank(tf) - 2)
    if d <= soft_max:
        return 1.10 - 0.18 * ((d - _SL_REASONABLE_IDEAL_ATR) / max(soft_max - _SL_REASONABLE_IDEAL_ATR, 1e-9))
    floor = 0.52 if _tf_rank(tf) >= 4 else 0.34
    excess = d - soft_max
    return max(floor, 0.92 / (1.0 + 0.075 * excess * excess))
...
def _sl_pool_score(target: Any, *, entry: float, atr: float, side: str) -> Tuple[float, float, float, List[str]]:
    ...
    reasonable_mult = _sl_reasonable_distance_mult(dist_atr, tf)
    score = sig * struct * fresh * touch * tf_mult * distance_mult * reasonable_mult
    ...
    if reasonable_mult < 0.92:
        notes.append(f"reasonable-distance×{reasonable_mult:.2f}")
...
def diagnose_tp_pools(
    ...
    for target in pools:
        ...
        try:
            ...
            _dist_mult = _tp_reasonable_distance_mult(row.distance_atr, tf, terminal_target)
            if _dist_mult < 0.90:
                row.notes.append(f"distance-adjusted×{_dist_mult:.2f}")
...
def _target_from_report_row(snap: Any, role: str, side: str, row: PoolCandidateDiagnostic) -> Optional[Any]:
    """Resolve a diagnostic row back to its live PoolTarget without rescoring.

    select_*_with_report() used to run diagnose_* and score_* separately, which
    duplicated every pool gate. The diagnostic pass now owns the single selector
    decision; this helper only maps the selected audit row back to the original
    target object for execution.
    """
    ...
    if role_u == "TP":
        pools = list(_safe(snap, "bsl_pools", []) or []) if side_l == "long" else list(_safe(snap, "ssl_pools", []) or [])
    elif role_u == "SL":
        pools = list(_safe(snap, "ssl_pools", []) or []) if side_l == "long" else list(_safe(snap, "bsl_pools", []) or [])
    else:
        pools = []
    for target in pools:
        pool = _safe(target, "pool", None)
        try:
            if abs(float(_safe(pool, "price", 0.0)) - float(row.pool_price)) > 1e-8:
                continue
            if str(_safe(pool, "timeframe", "")) != str(row.timeframe):
                continue
            row_side = str(row.pool_side or "").upper()
            pool_side = _pool_side(pool).upper()
            if row_side and pool_side and row_side != pool_side:
                continue
            return target
        except Exception:
            continue
...
def select_tp_with_report(
    ...
    The diagnostic pass is the selector pass; do not run score_tp_pools() again.
    This keeps TP gates single-application and prevents tiny timing/data drift
    between report and execution selection.
    """
    ...
    target = _target_from_report_row(snap, "TP", side, row)
    if target is None:
        report.summary = f"selected TP row could not be resolved to live pool target; {report.summary}"
        return None, None, None, report
    score = PoolScore(
        target=target,
        ...
        rr=float(row.rr),
        sweep_prob=float(row.sweep_prob),
        raw_score=float(row.sweep_prob),
        confluence=float(row.confluence),
        gauntlet_n=int(row.gauntlet_n),
        gauntlet_pen=max(0.0, min(1.0, 1.0 - _GAUNTLET_PENALTY_PER * int(row.gauntlet_n))),
        ev=float(row.ev),
        components={
            "selection_ev": float(row.selection_ev),
            "delivery_prob": float(row.delivery_prob),
            "required_delivery_prob": float(row.required_delivery_prob),
            "rr_floor": float(row.required_rr),
            "cost_r": float(row.cost_r),
            "be_move": float(row.be_move),
            "reasonable_distance": _tp_reasonable_distance_mult(
                float(row.distance_atr), str(row.timeframe), float(row.distance_atr) > _max_reach_for_tf(str(row.timeframe))
            ),
        },
        reasons=list(row.notes or [row.reason]),
    )
    return float(row.tp_price), target, score, report
...
def select_sl_with_report(
    ...
    The audit pass is also the selector pass, so the protective SL criteria are
    evaluated once and the selected row is mapped back to the source PoolTarget.
    """
    ...
    if row is None:
        return None, None, None, report
    target = _target_from_report_row(snap, "SL", side, row)
    if target is None:
        report.summary = f"selected SL row could not be resolved to live pool target; {report.summary}"
        return None, None, None, report
    reasons = list(row.notes or [row.reason])
    if "outside-liquidity-zone" not in reasons:
        reasons.append("outside-liquidity-zone")
    pick = SLPoolPick(
        target=target,
    ...
    return float(row.sl_price), target, pick, report
