...
class QuantStrategy:
    ...
    def _defer_entry_signal(self, signal, reason: str, cooldown_sec: float = 30.0) -> None:
        try:
            self._last_entry_defer = {
                "ts": time.time(),
                "side": str(getattr(signal, "side", "") or ""),
                "entry_type": self._signal_entry_type_value(signal),
    ...
    def _evaluate_entry(self, data_manager, order_manager, risk_manager, now):
        ...
        if signal is not None:
            ...
            if _surf_after is not None and not getattr(_surf_after, 'has_positive_edge', False):
                ...
                _refine_armed = False
                try:
                    if hasattr(self._entry_engine, "arm_refine_watch_from_signal"):
                        _refine_armed = bool(self._entry_engine.arm_refine_watch_from_signal(
                            signal,
                            f"target surface deferred: {_why}",
                            now=now,
                        ))
                except Exception as _re:
                    logger.debug(f"TargetSurface refine-watch arm failed: {_re}")
                if _refine_armed:
                    _why = f"{_why}; refine watch armed for pullback/risk compression"
        ...
        if now - self._last_think_log_v2 >= 30.0:
            ...
            try:
                _defer = getattr(self, '_last_entry_defer', None)
                if isinstance(_defer, dict):
                    ...
                    if 0.0 <= _age <= max(30.0, _cooldown + 30.0):
                        ...
                        _remain = max(_cooldown - _age, 0.0)
                        parts.append(
                            f"RoutePaused={_side} {_typ} {_why}"
                            + (f" ({_remain:.0f}s)" if _remain > 0 else "")
                        )
            except Exception:
                pass
