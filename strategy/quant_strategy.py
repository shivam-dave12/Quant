...
class QuantStrategy:
    def __init__(self, order_manager=None, instrument=None):
        ...
        # Effective leverage is selected per trade so RISK_PER_TRADE is measured
        # against margin at SL, not leveraged notional. Reset after each entry/close.
    ...
    def _apply_cross_asset_overlay(self, signal, price: float, atr: float) -> None:
        ...
        try:
            ...
            if not bool(getattr(adj, "entry_allowed", True)):
                _br = str(getattr(adj, "block_reason", "") or getattr(adj, "reason", "") or "cross-asset portfolio exposure conflict")
                setattr(signal, "cross_asset_block_reason", _br)
            logger.info(
    ...
    def _institutional_signal_veto(self, signal, price: float, atr: float, ict_ctx) -> str:
        ...
        try:
            ...
            cross_block = str(getattr(signal, "cross_asset_block_reason", "") or "").strip()
            if cross_block:
                return f"cross-asset exposure conflict: {cross_block[:180]}"
        except Exception as e:
    ...
    def _institutional_decision_matrix(self, signal, ict_ctx, flow_state,
        ...
        if not liq_ok:
            ...
            allows.append(f"SL before liquidation guard ${liq_guard:,.1f} @ {_liq_preview_lev:.0f}x")
        ...
        if _in_pos:
            ...
            try:
                if pos.entry_price > 0 and pos.quantity > 0:
                    ...
                    _lev = max(float(getattr(pos, "entry_leverage", 0.0) or QCfg.LEVERAGE()), 1.0)
    ...
    def _record_pnl(self, pnl: float, exit_reason: str = "unknown",
        ...
        try:
            ...
            if _entry_px > 0 and _qty > 0:
                ...
                _lev_f = max(float(getattr(pos, "entry_leverage", 0.0) or QCfg.LEVERAGE()), 1.0)
    ...
    def _compute_quantity(self, risk_manager, price,
        ...
        Margin guard: computed margin must stay inside the configured margin budget.
        """
    ...
    def format_status_report(self):
        ...
        if not p.is_flat() and p.entry_price > 0 and p.quantity > 0:
            try:
                ...
                _rpt_lev = max(float(getattr(p, "entry_leverage", 0.0) or QCfg.LEVERAGE()), 1.0)
