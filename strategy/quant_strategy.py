    ...
    def _enter_trade(self, data_manager, order_manager, risk_manager, side, sig, mode="reversion",
        ...
        if protected_cross:
            ...
            if side == "long" and ask_px > 0:
                ...
                protected_cross_applied = True
        if protected_cross and not protected_cross_applied:
            protected_cross = False
            native_market_bracket = False
        ...
        if not _using_force_levels:
            ...
            if protected_cross and _sig_is_valid:
                try:
                    signal_ref = _round_to_tick(_sig_entry)
                    signal_sd = abs(signal_ref - sl_price)
                    signal_td = abs(tp_price - signal_ref)
                    cross_sd = abs(entry_ref - sl_price)
                    cross_td = abs(tp_price - entry_ref)
                    signal_dir_ok = (
                        (side == "long" and sl_price < signal_ref < tp_price)
                        or (side == "short" and tp_price < signal_ref < sl_price)
                    )
                    cross_dir_ok = (
                        (side == "long" and sl_price < entry_ref < tp_price)
                        or (side == "short" and tp_price < entry_ref < sl_price)
                    )
                    if signal_dir_ok and cross_dir_ok and signal_sd > 1e-10 and cross_sd > 1e-10:
                        signal_rr = signal_td / signal_sd
                        cross_rr = cross_td / cross_sd
                        max_degradation = float(
                            getattr(config, "ENTRY_PROTECTED_CROSS_MAX_RR_DEGRADATION", 0.18)
                        )
                        max_degradation = max(0.0, min(0.75, max_degradation))
                        min_live_rr = signal_rr * (1.0 - max_degradation)
                        if signal_rr > 0.0 and cross_rr + 1e-9 < min_live_rr:
                            old_entry_ref = entry_ref
                            limit_px = signal_ref
                            entry_ref = signal_ref
                            use_maker = True
                            protected_cross = False
                            native_market_bracket = False
                            mt_reason = (
                                f"protected_cross_declined_rr: live_rr={cross_rr:.2f} "
                                f"< required {min_live_rr:.2f}; maker_at_signal={entry_ref:.1f} "
                                f"(cross={old_entry_ref:.1f})"
                            )
                            logger.info(f"Entry routing repriced: {mt_reason}")
                except Exception as _rr_guard_err:
                    logger.debug(f"Protected-cross R:R guard skipped: {_rr_guard_err}")
