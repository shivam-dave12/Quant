"""
Multi-timeframe structural liquidity map.

The map detects, ranks and updates BSL/SSL pools from closed candles across
1m/5m/15m/1h/4h/1d. It produces fresh sweep events and opposing liquidity
targets for the sole ICT/Liquidity entry authority. No external alpha score,
profile model or legacy bridge can modify its pool geometry.
"""
...
# should not vanish from the map; downstream sweep/execution models
...
class LiquidityPool:
    ...
    def significance(self) -> float:
        ...
        raw = (base + touch_bonus + freshness_penalty) * htf_mult
        return max(0.1, raw)
...
class LiquidityMap:
    """
    Multi-timeframe structural liquidity pool map.
    ...
    def reset_snapshot(self) -> None:
        """Invalidate cached pool state after a completed position cycle."""
    ...
    # Structural update: pool detection, confluence, sweep tracking and consumption
    # ─────────────────────────────────────────────────────────────────────
    ...
    def update(
        ...
        Steps executed in order:
          ...
          3. Detect sweeps on all timeframes from the last CLOSED candle
          4. Promote SWEPT -> CONSUMED for pools price has moved beyond
          5. Maintain rolling fresh sweep history
