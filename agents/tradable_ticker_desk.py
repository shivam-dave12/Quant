"""Institutional tradable-selection desk.

This desk is deliberately *pre-candle*.  It ranks the full discovered catalog
using cheap venue metadata and live ticker snapshots, then hands only a small
shortlist to the expensive strategy desks that open candle/orderbook/trade
streams.  The goal is to avoid subscribing to every exchange product just to
learn that most of them are not worth analysing right now.
"""
...
class TradableDeskSelection:
    ...
    def compact_text(self, limit: int = 12) -> str:
        lines = ["DYNAMIC TRADABLE DESK"]
        lines.append(f"selected={len(self.selected)} parked={len(self.parked)}")
        for row in list(self.rows)[:max(0, int(limit))]:
            tag = "ACTIVE" if row.selected else "PARK"
            lines.append(
                f"{tag} {row.rank:02d} {row.asset_id:<18} {row.venue.upper():<10} "
                f"score={row.score:.2f} spread={row.spread_bps:.1f}bps "
                f"liq={row.turnover_score:.2f} vol={row.volatility_score:.2f} {row.reason}"
            )
        if self.notes:
...
class TradableTickerDesk:
    ...
    def select(
        ...
        *,
        ...
        """Return the shortlist that should own live candle/data subscriptions.

        `protected_ids` are instruments with live/entering/exiting positions; they
        remain selected even if their current opportunity score falls.
        """
        ...
        rows: list[tuple[TradableInstrument, TradableDeskRow]] = []
        for inst in instruments:
            row = self._score_instrument(inst, delta_tickers=delta_tickers, icici_quote=icici_quotes.get(str(inst.asset_id)))
        ...
        for rank, (inst, row) in enumerate(rows, 1):
            ...
            out_rows.append(TradableDeskRow(
                ...
                icici_details=row.icici_details,
            ))

        parked = [inst for inst, _ in rows if inst.asset_id not in selected_ids]
    ...
    def _load_icici_quote_probes(self, instruments: Sequence[TradableInstrument], icici_api: Any, notes: list[str]) -> Dict[str, dict]:
        """Probe only a small ICICI shortlist for quotes/details.

        This keeps the desk institutional: Security Master gives cheap coverage;
        authenticated quote calls are rate-limited and bounded, never fired for
        the full Indian catalog.
        """
        ...
        if limit <= 0:
            return {}
        candidates = [inst for inst in instruments if inst.primary_exchange == ExchangeName.ICICI]
        if not candidates:
            return {}
        out: Dict[str, dict] = {}
        failures = 0
        for inst in candidates[:limit]:
            try:
                ...
                failures += 1
                if failures == 1:
                    notes.append(f"icici quote preflight skipped/failed: {exc}")
                break
        if out:
            notes.append(f"icici authenticated quote probes={len(out)}")
        return out

    def _score_instrument(self, inst: TradableInstrument, *, delta_tickers: Mapping[str, dict], icici_quote: Optional[Mapping[str, Any]] = None) -> TradableDeskRow:
        ...
        price = self._first_float(merged, (
            "mark_price", "markPrice", "spot_price", "last_price", "lastPrice", "close", "close_price", "price"
        ))
        ...
        leverage_score = clamp(float(getattr(inst, "max_leverage", 0.0) or 0.0) / 100.0)
        ...
        # Institutional prefilter: liquid + moving + executable beats simply being
        # present in a catalog.  ICICI coverage is allowed for discovery/details,
        # but it is not ranked like a live execution venue until the runtime is on.
        score = clamp(
            0.25 * turnover_score +
        ...
        if primary.exchange == ExchangeName.ICICI:
            reasons.append("icici_coverage")
        if not reasons:
            reasons.append("catalog_rank")
    ...
    def _first_float(row: Mapping[str, Any], names: Iterable[str]) -> float:
        for name in names:
            if name in row:
                ...
                if f > 0:
                    return f
        # shallow nested fallback for common product/ticker payloads
    ...
    def _turnover_proxy(self, row: Mapping[str, Any], price: float) -> float:
        direct = self._first_float(row, (
            "turnover", "turnover_usd", "turnoverUsd", "quote_volume", "quoteVolume",
            "volume_usd", "volumeUsd", "notional_volume", "notionalVolume"
        ))
        if direct > 0:
            return direct
        vol = self._first_float(row, ("volume", "volume_24h", "volume24h", "base_volume", "baseVolume"))
    ...
    def _volatility_score(self, row: Mapping[str, Any], price: float) -> float:
        ...
        if high > 0 and low > 0 and price > 0 and high > low:
            rng = (high - low) / price
            # Sweet spot: enough movement to matter, not chaos-only selection.
            return clamp(1.0 - abs(rng - 0.035) / 0.07)
        change = abs(self._first_float(row, ("price_change_percent", "priceChangePercent", "change_percent", "changePercent")))
        if change > 0:
    ...
    def _execution_score(self, inst: TradableInstrument) -> float:
        ...
        if primary in (ExchangeName.DELTA, ExchangeName.COINSWITCH):
            base = 1.0
        elif primary == ExchangeName.ICICI:
            base = 0.78 if bool(_cfg("ICICI_ENABLED", False)) else 0.42
        else:
            base = 0.35
        if len(exs) > 1:
            base += 0.08
    ...
    def _max_spread(self, inst: TradableInstrument) -> float:
        ...
        if "option" in ac:
            return float(_cfg("FUND_MAX_SPREAD_BPS_OPTION", 120.0))
        if "future" in ac:
