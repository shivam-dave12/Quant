...
INSTITUTIONAL_FRACTIONAL_KELLY = 0.60  # legacy compatibility only; not an active sizing authority
INSTITUTIONAL_SIZING_METHOD = "fixed_dollar_invalidation_risk_until_calibrated_barrier_model"
INSTITUTIONAL_FIXED_RISK_DEPLOYMENT_SCALAR = 1.0
...
STARTUP_EXTERNAL_EXPOSURE_RECHECK_SEC = 10.0  # keep analytics live; periodically re-authorise entries when exposure clears
...
# V23: protection levels are selected from a scored multi-timeframe liquidity
# map, not nearest 1-minute candle bounds. A live directional trade must have
# both an observable invalidation pool and an economically reachable objective.
LIQUIDITY_POOL_CLUSTER_MERGE_BPS = 2.0
ADAPTIVE_PROTECTION_REQUIRE_MTF_INVALIDATION_POOL = True
ADAPTIVE_PROTECTION_REQUIRE_MTF_OBJECTIVE_POOL = True
ADAPTIVE_PROTECTION_MIN_NET_TARGET_AFTER_COST_BPS = 2.0
...
# V23 cost authority: route total already includes configured round-trip fee
# plus route friction. The bracket layer adds only an explicit adverse exit-tail
# reserve; it never doubles the composite route cost.
BARRIER_OUTCOME_ROUTE_COST_AUTHORITY = "full_cycle_route_cost_plus_explicit_exit_tail_reserve"
BARRIER_OUTCOME_ROUND_TRIP_COST_MULTIPLIER = 1.0  # retained compatibility key; must remain 1.0
BARRIER_OUTCOME_EXIT_TAIL_RESERVE_BPS = 1.0
BARRIER_OUTCOME_EXIT_TAIL_RESERVE_BPS_BY_ASSET = {
    "BTC": 0.75, "OIL": 1.25, "NATGAS": 2.00,
    "GOLD_PAXG": 1.00, "GOLD_HL": 1.00,
    "SILVER_SLVON": 3.00, "SILVER_XAG": 2.50, "SILVER_HL": 2.00,
}
...
PREDICTIVE_LABEL_MIN_SPACING_SEC = 60.0
