    ...
    def start(self):
        ...
        while self.running:
            try:
                ...
                # Telegram-only stop policy: terminal Ctrl-C / SIGINT must not
                # stop the controller or the trading bot. Use Telegram /stop.
...
def main():
    ...
    def _signal_handler(signum, frame):
        ...
        # Do NOT call sys.exit(), controller.stop(), or bot_instance.stop() here.
        # External SIGTERM/SIGINT can be generated by docker/systemd/terminal; the
        # bot must keep running until an authorised Telegram /stop command is sent.
        # os.write is async-signal-safe enough for this emergency breadcrumb and
        # avoids the previous "I/O operation on closed file" logging traceback.
        try:
            _os.write(2, f"Signal {signum} ignored; use Telegram /stop to stop bot\n".encode("utf-8", "replace"))
        except Exception:
            pass
    ...
    try:
        ...
        try:
            ...
            logger.exception("Controller restart after KeyboardInterrupt failed")
            raise
    except Exception as e:
